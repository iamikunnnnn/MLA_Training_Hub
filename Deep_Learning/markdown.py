def basic():
    markdown = r"""
# 1 基本知识
1. 首先理解神经网络的本质是一个庞大的函数,里面的神经元是一个个小函数,最终的代价函数为输出层的值与实际值的误差
2. 其中的核心是：权重$w_{i,j}$、激活值(or输出值)$a_i$、线性输出值$z_i$、偏置$b_i$、输入值$x_{i,j}$，每个神经元都是由$w_{i,j}$ $a_i$ $b_i$ $x_{i,j}$组成的(即$a_i=\sigma(w_{i,j}*x_{i,j}+b_i)$)，并且当前层的输出$a$是下一层的输入$x$
    (j表示上一层神经元数量，i表示当前层神经元数量
    - $x$的数量取决于上一层输出的数量，每一层一共有$i*j$个$x$，表示为$x_{i,j}$
    - 权重(或者叫参数)$w_{i,j}$决定了神经网络的实际效果，$w_{i,j}$也是需要优化的目标，和$x$一样，一共有$i*j$个$w$
    - 激活值$a$是神经网络中每一层输出的的值，传播时循环输入到下一次的神经元作为$x$，每一个$a_i$都会作为每一个神经元的输入作为$x_{i,j}$
    - $z_i$表示每层的原始输出，最终输出$a=\sigma(z)$,$\sigma$为激活函数,$z_i$=$w_{1,j}*x_{1,j}+b_1$
    - 偏置$b_i$表示了激活阈值调整项，当$b_i$较小甚至为负数时，需要更大的$w_{i,j}$才能激活神经元，难度更高。(这里的默认假设是w通常是为正的)
3. 在神经网络中分代价函数(cost function)和损失函数(loss function)，
    - 损失函数表示单个样本代入代价函数时的具体损失
    - 代价函数包括总体的每个样本的损失函数，最后的值通常取所有样本代入后计算的损失函数结果的平均
    - 重点：所以每一次参数更新都意味着要计算每个样本的损失(损失函数)的梯度，取梯度平均作为代整个代价函数的梯度，并以此更新整个神经网络
    """
    return markdown
def progress():
    markdown = r"""
